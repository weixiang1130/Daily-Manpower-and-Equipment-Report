...
> 本專案為通用工具，`app.js` 內建的工地／分包商／人員名單皆為範例佔位資料。實際名單請放在地端 `config.local.js`（已列入 `.gitignore`，格式見 `app.js` 開頭註解）或於系統「設定」頁輸入；業務資料存於雲端共用資料庫（或地端部署的資料目錄），不會回寫至本程式碼庫，避免客戶專案資訊外流。

## 功能總覽

- **多工地完全隔離（Multi-Project Isolation）**：每個工地一份獨立的資料環境（名單池與紀錄互不共用，v17 起名單一律各站自建）；頁首切換工地即切換整個資料環境（Context Switch），總覽頁跨工地彙總並可點擊切換
...
- 資料存於雲端共用資料庫（Netlify Functions + Blobs），所有使用者即時共編；同一筆紀錄的並發編輯以版本檢查（baseV/409）保護——後送出者會收到衝突提示需重填，不會靜默互相覆蓋；但不留「誰修改」的軌跡，需要問責到人時建議升級至含使用者登入的後端
