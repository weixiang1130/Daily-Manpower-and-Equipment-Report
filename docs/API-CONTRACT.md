...
2. **版本檢查**：`baseV`＝前端載入當下的版本。**v18 補充（前端紀律）**：表單開啟當下即快照 baseV，之後任何背景刷新都不得更新該快照——送出一律用快照值，確保「使用者所見版本」與「宣告基準版本」一致，背景刷新不會讓寫入繞過 409。**v18 補充（前端紀律）**：表單開啟當下即快照 baseV，之後任何背景刷新都不得更新該快照——送出一律用快照值，確保「使用者所見版本」與「宣告基準版本」一致，背景刷新不會讓寫入繞過 409
