...
- `config` 不存在時該站回 `null`
- 地端且讀取者為系統管理者時，`master` 另含 `adminDepartments` 與 `siteGrants`（§4.1）；
  `siteGrants` **不回給一般使用者**
...
- **`siteGrants`（節點 55，選填，地端限定）**：跨工地授權覆寫，形狀見 §4.1。
  與 adminDepartments 同守則——**省略＝保留既有值**；要清空請明確送空物件 `{}`。
  後端正規化（工號 trim、工地去重）後整包覆蓋儲存；**不驗工地是否存在**
  （允許先授權、後納管），打錯字的項目在授權判定時自然不生效並由 `/whoami` 標示。
...
### 4.1 master
`{ "sites": string[], "adminDepartments"?: string[], "siteGrants"?: {…} }` — 全域設定。
`sites` 為工地清單，順序即顯示順序。
