...
{ "op":"master", "sites":["工地A","工地B"], "adminDepartments":["成本管理部"],
  "siteGrants": { "K0000000": { "sites":["工地B"], "by":"設定頁", "at":"2026-08-24", "why":"支援原因" } } }
```
- `sites` 必須為非空陣列，否則 `400`。成功回 `{ "ok": true }`
- **`adminDepartments`（v23.2，選填）**：省略時**保留既有值**，不是清空。
- **`siteGrants`（節點 55，選填，地端限定）**：跨工地授權覆寫，形狀見 §4.1。
- **`siteLeads`（節點 61，選填，地端限定）**：工地主管白名單，形狀同上，見 §4.1。
- 上列兩份「工號→工地」設定**共用同一守則**：與 adminDepartments 一樣
  **省略＝保留既有值**；要清空請明確送空物件 `{}`。後端正規化（工號 trim、工地去重）
  後整包覆蓋儲存；**不驗工地是否存在**（允許先授權、後納管），打錯字的項目在授權判定時
  自然不生效並由 `/whoami` 標示。
...
### 4.1 master
`{ "sites": string[], "adminDepartments"?: string[], "siteGrants"?: {…}, "siteLeads"?: {…} }` — 全域設定。
`sites` 為工地清單，順序即顯示順序。
...
**`siteGrants`（節點 55，地端限定）**：跨工地授權覆寫——ERP 專案角色之外，
由成本管理部額外授予特定工號的可見工地。形狀：

```json
{ "工號": { "sites": ["工地名", "…"], "by": "核准人", "at": "YYYY-MM-DD", "why": "原因" } }
```

- **只加工地、不升角色**：被授予者以工地使用者身分進入；無任何 ERP 工地角色者
  也能純靠覆寫進入（視為工地使用者）。撤銷 ERP 本有的權限請回 ERP，本欄位只加不減
- `sites` 必要；`by`／`at`／`why` 為稽核註記（本欄位沒有其他軌跡，是唯一紀錄）
- **僅在讀取者為系統管理者時回傳**（含工號與核准註記，不讓一般使用者列舉）；
  雲端後端無此欄位
- 工地名以資料庫 `sites.name` 為準比對（大小寫不敏感）；對不上的項目不生效，
  並在 `/whoami` 以 `grantsNotMatched` 標示

**`siteLeads`（節點 61，地端限定）**：工地主管白名單——指定工號可刪除**特定工地**的
「已回報」單據（計價依據，平時僅限系統管理者）。形狀與 `siteGrants` **完全相同**。

- **只給「刪該站已回報單」一項**：不授予可見性、不升角色、不開放設定與清空
- ⚠ **只在該工號本來就看得到的工地生效**（判定時與可見工地取交集）——
  白名單不是可見性授權，要讓人看到工地請用 `siteGrants`
- ⚠ **鎖檔優先**：鎖檔區間內的單，白名單成員一樣刪不掉（結算凍結是實質控制）
- **不取自 ERP 角色**：實查顯示 ERP 的 `Director` 含總部幕僚且涵蓋全部專案，
  與「該工地負責人」語意不符，故改由成本管理部於設定頁逐人維護
- 與 `siteGrants` 同樣**僅回傳給系統管理者**；`/whoami` 另以 `leadSites` 與
  `canDeleteReportedScope` 標示**該次請求者自己**的可刪範圍
...
| guideWork | number\|null | **引導人員出工數(工)**（v24.16 節點 64）。堆高機等機具隨車的引導（指揮）人員，屬「人」的口徑、以工計（可 0.5）。**null＝未填**（本單無引導人員）、0＝真的 0——報表據此決定留白或印 0，兩者不可壓成同值（同 `diff` 的理由）。全部機具類型皆選填 |
| guideOt2 | number\|null | **引導人員加班時數(前2小時)**（v24.16）。引導人員是「人」，加班**沿用點工的前2小時／第3小時起分段**（費率不同）——與機具本身的 `otHours`（單一欄不分段）互不相干、不相加 |
| guideOtOver | number\|null | **引導人員加班時數(第3小時起)**（v24.16） |
| guideNote | string | **引導人員備註**（v24.16，選填）。彙總口徑：引導三欄為人·工／時數，與機具的台·日分欄並存**不相加**（計價彙總、機具榜、工程師機具榜皆獨立欄呈現） |
...
> **節點 63 補充（地端權限模式下的稽核資料形狀）**：
> ① **讀取**——無稽核權限者（非成控／管理員）GET 得到的每筆 audits 元素為**瘦身摘要**
>   `{id, auditedAt, applied, actualCount, diff}`；items／note／attachments／auditor／statusAtAudit
>   一律不下發。摘要足供前端計算「稽核差異」標記與回報送出警示，敏感內容仍真隔離。
> ② **寫入**——無稽核權限者的 `op:record` **不覆寫稽核子層**：伺服器忽略 payload 的 audits，
>   資料庫既有稽核列與其附件描述資料原地保留。⚠ 這同時是資料保全修復——「整筆覆寫」語意
>   原本會讓工地端以（當時是空陣列的）audits 蓋掉成控的完整稽核；修正前任何工地端編輯
>   被稽核過的單，稽核紀錄即被清除。
> ③ **刪除**——無稽核權限者對**已有稽核紀錄**的單呼叫 `op:deleteRecord` 一律 403
>   （刪單會經 FK CASCADE 連坐銷毀稽核；「刪掉重開」原是讓稽核差異無痕消失的漏洞）。
> ④ **稽核附件**——`GET ?attachment=`／`op:uploadAttachment`（覆寫既有）／`op:deleteAttachment`
>   對 parent_kind 為稽核類的附件，無稽核權限者一律 403（隔離不再只靠附件 id 保密）。
> ⑤ 伺服器對 payload 稽核列缺 `auditor` 者（瘦身摘要形狀，升權窗口的舊快取）整批改走
>   保留路徑，不進 InsertAudits（該處會逐列跳過＝靜默滅失）。
> ⑥ 雲端／`Auth:Mode=Off` 無身分可辨，維持原行為（全量下發、payload 覆寫、無上述守衛）。
...
出工日＋**windowDays 個工作天**內（含最後一天；週六、週日不計，
**國定假日不排除**——需要的話日後把假日清單放進同一把設定下發）可送出回報；
之後該單對工地承辦**鎖定**，由**該站工地主管**（節點 61 白名單，
`Authz.CanLeadOverride`）或管理員代為回報，回 403 `{"error":"forbidden"}`。
工作天計算＝前端 `addWorkdays()`／後端 `AddWorkdays()` 同一口徑
（使用者 2026-09-10 更正：是工作天，不是日曆天）。
