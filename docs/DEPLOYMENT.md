...
> **v24.9 起改用「遷移包」，不再是逐項手工搬運。**
> 舊版流程要求管理員下載 JSON、再依 `attachments[].id` **逐一**下載附件、
> 又另外匯出費率書——線上實測附件有 831 個／104.6 MB，逐一下載並不可行，
> 且三樣分開搬很容易漏掉其中一樣。現在一次打包完成。

### ① 成本管理部：匯出遷移包

1. **公告停機時段**，並於設定頁的「鎖檔區間」把「最早 ～ 今天」設為鎖定、
   **立即生效**並「套用到全部工地」——避免匯出期間仍有人寫入舊站而漏搬。
2. 設定頁 → 管理員登入 → 資料管理 → **「📦 切換日遷移包（含附件・給資訊處）」**。
   附件多時會跑數分鐘，畫面有進度；卡住可按「取消匯出」後重跑。
3. 產出 `點工機具_切換日遷移包_YYYY-MM-DD.zip`，交付資訊處。

> 命令列替代方案（同樣結構）：`python backend/sql/export-cloud-bundle.py --base-url <站台> --user <帳號> --password <密碼> --out <輸出目錄>`
>
> ⚠ 設定頁的「下載資料備份（JSON・不含附件）」**不是搬家用的**——依合約它不含
> 附件本體與費率書，只帶那份過去會變成「附件描述都在、檔案全部打不開」。

### ② 資訊處：匯入（包內附 `匯入說明_給資訊處.md`）

遷移包結構：

```
data.json              全部單據＋各站名單池
rates.json             行情通報費率書
...
1. **建庫**：全新環境跑 `DB-SCHEMA.sql`；已建過庫則依序跑三支 `ALTER-*.sql`（見 §3.5 ②）
2. **匯入單據**：`python backup-json-to-sql.py data.json import.sql` 後以 `sqlcmd` 匯入
3. **放置附件**：把 `attachments/` 底下**所有檔案**複製到 `App:AttachDir`，**檔名不要更動**
   （轉換工具已寫好 `attachments.file_path`，檔案就位即可下載，不需回頭改資料庫）

> 費率書目前系統不使用（計價模組已於 v24.5 下架），銜接時不必匯入；
> 包內仍保存，日後要啟用計價時再處理。

### ③ 前端的名單檔（**不在任何交付包裡，必須另外遞交**）

`frontend/config.local.js` 含真實工地名、分包商、工程師名單與管理員密碼，
**不進版控**，因此不會出現在原始碼交付包或更新包中。
請由成本管理部另行遞交，放到 `App:StaticDir` 所指的資料夾（與 `index.html` 同層）。

> ⚠ 這個檔案缺席時**畫面照樣開得起來、不會報錯**，但管理員密碼會退回程式預設值，
> 且各工地名單池會是空的。部署後請以 F12 → Network 確認 `config.local.js` 為 **200 而非 404**。
...
### ④ 驗證清單（缺一不可）：
   ...
   - **代辦扣抵**：任選一筆有代辦的單，確認「廠商排名」頁籤的代辦扣抵金額算得出來
     （算不出來多半是費率書沒搬，見步驟 4）
   - 管理員登入 → 設定頁名單維護可正常存檔（確認 `config.local.js` 已就位）
### ⑤ 切換與退場

- 以 `MANIFEST.json` 的筆數對照資料庫 `SELECT COUNT(*)`，逐表核對
- DNS／內網入口改指地端；通知使用者新網址
- **退場舊站**：更換或停用 Netlify 站台密碼、（建議）刪除 Netlify 站台與環境變數，
  避免兩份資料並存造成誤填
- 舊站退場前，建議把最後一次的遷移包留存一份作為切換點的快照
...
- **資料層起點包已備妥**：[`backend/sql/`](../backend/sql/README.md) 內含經實測的 SQL Server 建表 DDL（19 表＋5 VIEW）、備份 JSON→SQL 遷移工具、逐操作 SQL 對照與並發寫法——**請從 backend/sql/README.md 讀起**，不必從零設計 schema
