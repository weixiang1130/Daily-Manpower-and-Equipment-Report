...
使用者決定將系統交由公司資訊部門部署至公司自有伺服器。IT 需要「環境與架構」交付物。盤點後的缺口：repo 內的 `backend/cloud/functions/api.mjs`（資料 API）與 Netlify Blobs（儲存）是 Netlify 專用執行環境——IT 取得 repo 只能架出靜態頁面，API 會是死的。另確認 GitHub 上使用者看到的「.netlify」實為 `netlify/` 程式碼目錄（Edge Function＋Function），`.netlify/` 狀態資料夾從未入版控（.gitignore 有擋），無機密外洩問題。

## 變更內容

1. **`backend/onprem/server.mjs`** — 可攜式伺服器（零外部依賴，Node 18+，不需 npm install）：
   - 與 `backend/cloud/functions/api.mjs` **同一 API 合約**（GET ?site=/scope=all；POST 7 ops 含 409 樂觀並發、id 格式驗證、addOption 白名單）
   ...
   - 前端 app.js 完全不需修改
2. **`backend/onprem/import-backup.mjs`** — 資料遷移工具：把管理員「下載完整備份（JSON）」的輸出灌入地端資料目錄（master＋各站 config＋逐筆紀錄，id 格式不符者跳過並統計）
...
- API 合約與 `backend/cloud/functions/api.mjs` 逐 case 比對（master/config/record/addOption/deleteRecord/clearSite/clearAll、409 條件、id regex、POOLS 白名單一致）
