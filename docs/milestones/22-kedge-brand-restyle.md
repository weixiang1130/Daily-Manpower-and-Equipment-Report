...
## v16.2 補強（2026-07-29，排版緊湊度回饋）

- **根因**：欄位過寬不只是格線設定，更是**標籤文字過長**（數字欄標籤達 39–57 字元）撐開欄位。縮短為 2–6 字，完整語意改放 `title` 屬性（滑鼠懸停可見），資訊零遺失
- **尺寸全面收斂**：`.field` span 6→4、`.field-num` span 3→2（框 190→130px）、gap 14/18→10/14px、欄內 gap 6→4px、panel padding 18/20→14/16px、input padding 10/12→7/10px、label 13→12.5px
- **實測成效**（1600×900）：申請表單四個短欄位併為一列；回報表單數字欄 3 列→2 列；表單總高 923→745px（−19%）；欄寬階層 數字 187px ＜ 文字 388px ＜ 說明 1191px
- 手機版（≤720px）覆蓋規則確認有效：單欄、輸入框全寬、無橫向溢出
- 驗證：申請/回報全流程、稽核表單（4 數字欄併一列）、console 零錯誤

## v16.3 補強（2026-07-29，歷程報表分頁）

- 歷程報表明細沿用清單的 `paginate()`／`bindPager()` 機制（`listPage.report`），每頁 10 筆；期間/快捷鍵/廠商/內容/工程師/報表類型切換皆重置為第 1 頁
- **匯出安全性**：`exportCSV`／`exportSummaryCSV` 各自呼叫 `def.rows()`／`pricingSummaryTable()` 取全量，與畫面分頁狀態完全解耦——實測「畫面停在第 2／3 頁時匯出 → CSV 仍為 29 筆全量」，月結計價依據不受影響
- 計價彙總不分頁（依廠商分組，筆數等同廠商數）
- 驗證：29 筆→3 頁、篩選回第 1 頁、機具/點工報表切換、CSV 全量、報表區零內部捲軸、console 零錯誤
