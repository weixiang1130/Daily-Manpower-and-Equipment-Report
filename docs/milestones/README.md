...
| [50](50-lock-ranges-daily-signer.md) | 鎖檔區間；月租逐日簽認工程師；逐日內容欄版面 | 鎖檔從「一個切點」擴充成**可累積的區間清單**：可多段、**可預約生效**（現在設定、8/20 17:00 起鎖 7/1～8/17）、可停用解鎖、各工地獨立、管理員限定。前端 9 處攔截一行未動（判定集中在 `isLockedDate()`），**地端後端再擋一次**——⚠ 修改既有單據時新舊日期都要查，否則改個日期就搬出鎖定區間。月租加每日簽認工程師（每天簽的人不同，單頭 `checker` 改為本張單的覆核者）。**順手補齊節點 49 斷掉的遷移鏈**：遷移工具原本沒有 billing/租期/onSiteDays/逐日紀錄，切換日一搬 15 張月租單會全部退化成日租。18→19 表＋升級腳本 |
| [51](51-appsettings-first-config.md) | 部署設定改以 appsettings.json 為主 | 資訊處要求不用環境變數（多系統共存會撞名、墊高部署複雜度）。九處 `GetEnvironmentVariable` 收斂成統一讀取器，環境變數降為選用覆寫。⚠ 查證時抓到 **`ASPNETCORE_URLS` 對本交付包無效**——根層 `"Urls"` 載入順序晚於它、會靜默蓋掉，而部署手冊正是叫資訊處設那個環境變數，照做會只綁本機、全公司連不上且不報錯。改用獨立鍵 `App:Urls`，並新增三則「設錯會被點名」的守衛 |
| [52](52-one-click-bundle-cell-height.md) | 一鍵切換日遷移包；表格儲存格限高 | 設定頁一鍵打包單據＋費率書＋全部附件成 zip（結構與 `export-cloud-bundle.py` 一致）。⚠ 寫完才發現 app.js 早有 ZIP 寫入器（xlsx 用），已改用既有的。表格盤點：欄寬登記乾淨，真正問題是**列高由最長欄決定**——「逐日使用紀錄(月租)」1,446 字要 85 行、整列 1,359px。改成儲存格限高內捲（不動欄寬與內容，匯出仍全文），列高收斂到最高 120px |
...
| [61](61-sitelead-delete-reported.md) | 工地主管白名單：可刪指定工地的已回報單 | 不採 ERP Director（含總部幕僚、語意不符）改用設定頁白名單；只在已可見的站生效、鎖檔優先；前後端閘門必須成對否則功能形同虛設 |
| [62](62-agent-type-summary.md) | 代辦工種彙總表（責任歸屬廠商×工種） | 各工種費率不同、只有合計不知道扣什麼工；一份彙總邏輯三個出口；舊制單獨立列不假裝知道也不丟掉 |
| [63](63-audit-mismatch-flagging.md) | 稽核差異全系統警示＋篩選報表 | 差異標記算出來、自動滅（無解除欄位）；清單/歷程/全部匯出一個不漏；⚠ 順修工地端編輯會滅掉稽核紀錄的保全漏洞 |
| [64](64-equip-guide-personnel.md) | 機具回報引導人員＋工程師機具使用榜 | 引導人員是「人」的口徑（加班依點工分段）、與台·日分欄永不相加；null＝未填≠0；排名 CSV 新區塊固定附最後（既有區塊位置不能動） |
| [65](65-labor-3day-report-lock.md) | 點工三日回報鎖＋主管代處理權 | 只鎖「待回報→已回報」的送出（鎖編輯會堵死節點 63 的更正）；不溯及既往；代處理收斂成 `CanLeadOverride` 通則；新舊日期都查防改期繞鎖 |
| [66](66-dashboard-open-overview.md) | 總覽分區開放全員 | 未授權站改下發瘦身投影（口徑留在前端不重寫第二份）；卡片/排名全站、追蹤/列控/最近回報限自己；SITE_CACHE 重建必須保留 overviewOnly 否則等於全站解鎖 |
