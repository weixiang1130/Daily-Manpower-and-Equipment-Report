...
| [35](35-onprem-dotnet-api.md) | 地端 .NET 8 API — 階段 A／B／C（合約端點全數實作） | 以 .NET 8 Minimal API 實作合約 §2 與 §3 全部端點＋靜態前端；先補上 v22.8 漏掉的 SQL 落點（11→14 表）。逐欄比對 996 筆真實差異 0 處；409 以 UPDLOCK+HOLDLOCK 實作、12 執行緒並發實測恰好 1 人成功；附件本體落檔案系統、clearAll 範圍對齊雲端。含一則測試誤清本機測試庫的事故記錄 |
