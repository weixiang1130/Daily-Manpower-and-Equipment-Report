...
| [35](35-onprem-dotnet-api.md) | 地端 .NET 8 API — 階段 A（唯讀） | 以 .NET 8 Minimal API 實作合約 §2.1／§2.2 ＋靜態前端；先補上 v22.8 漏掉的 SQL 落點（11→14 表）。與雲端逐欄比對 996 筆真實差異 0 處，端到端開站計價數字與 SQL 對帳一致。未實作的操作一律回 501 |
